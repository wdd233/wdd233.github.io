...
yolo系列只有yolo v1明确提出了损失函数公式.对于YOLO这样一种讨喜的目标检测算法,就连损失函数.在v1中使用了一种sum-square error损失计算方法,就是简单差方相加而已.我们知道,在目标检测任务里,有几个关键信息是确定的:
(x, y)(w, h), class, confidence   
下面是v1的loss function
![YOLO_loss](/img/yolo_loss.png)  


v3对b-box进行预测的时候，对anchor中心点使用了BCE,为啥这么搞我还没研究透彻(Sigmoid+BCE)。对anchor的wh使用了MSE,这些在v3论文中没有明确说明,
